...
contract TrainingManager is ITrainingManager {
    ...
    mapping(uint256 trainingRunId => TrainingRunStatus status) private trainingRunStatuses;
    ...
    function getTrainingRunStatus(uint256 trainingRunId) external returns (TrainingRunStatus) {
        return trainingRunStatuses[trainingRunId];
    }

    function registerComputeNode(address account, string ipAddress, uint256 trainingRunId) external returns (bool) {
    ...
    function submitAttestation(address account, uint256 trainingRunId, bytes attestation) external returns (bool) {
    ...
    function getComputeNodesForTrainingRun(uint256 trainingRunId) returns (address[]) {
